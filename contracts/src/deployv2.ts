...
import { Vault, VaultDeployProps, TokenHolder, TokenA, VaultV2DeployProps, VaultV2, TokenHolderV2 } from './index.js';
...
let zkApp = new VaultV2(zkAppAddress);
...
async function deployTokenHolder() {
    try {
        ...
        let dexTokenHolder0 = new TokenHolderV2(zkAppAddress, zkToken0.deriveTokenId());
        let tx = await Mina.transaction(
            { sender: feepayerAddress, fee },
            async () => {
